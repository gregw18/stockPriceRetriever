...
from targetSecurity import TargetSecurity
...
class StockTarget:
    ...
    def _get_worksheet(self, srcFile, tabName):
        ...
        if os.path.isfile(srcFile):
            ...
            except (FileNotFoundError, PermissionError, ValueError, XLRDError) as E:
    ...
    def read_target_securities(self, srcFile, tabName):
        """
        Read in contents of requested tab in given spreadsheet.
        Convert each row to a stockTarget, add to a dictionary and return.
        """
        targets = {}
        myWs = self._get_worksheet(srcFile, tabName)
        if not (myWs is None):
